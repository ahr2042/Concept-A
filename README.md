...
| **Console layout** | One job per page: the side rail picks the source, the Pipeline page configures the chain and every knob, the Dashboard watches it run. The pipeline being assembled is one shared working config rather than a copy in each page's widgets, so the two deploy paths cannot disagree and a knob moved while streaming reaches the stream whichever page started it |
